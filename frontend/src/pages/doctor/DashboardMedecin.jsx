...
import {
  User, Calendar, Phone, MapPin, BookOpen, Stethoscope, TestTube, ClipboardList,
  FileText, Download, Mail, Bell, Settings, CheckCircle, Clock, UserCheck, AlertTriangle, TrendingUp
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex font-[Poppins] bg-gradient-to-br from-gray-100 to-gray-50">
      ...
      <main className="flex-1 ml-20 md:ml-60 p-6 space-y-8 transition-all duration-300">
        <h1 className="text-2xl font-bold text-[#0c4687]">Fiche Patient : {patient.nom} {patient.prenom}</h1>
        ...
        <div className="white-box p-6 space-y-3 overflow-x-auto">
          ...
          <table className="w-full text-sm mt-2">
            <thead className="text-left text-[#0c4687] border-b">
              <tr>
                <th>Date</th><th>Type</th><th>Valeur</th><th>Unité</th>
              </tr>
            </thead>
            <tbody>
        ...
        {/* Bloc 6 - Documents */}
        <div className="white-box p-6 space-y-3 overflow-x-auto">
          <h2 className="text-xl font-semibold flex items-center gap-2"><FileText size={20} /> Documents liés</h2>
          <table className="w-full text-sm mt-2">
            <thead className="text-left text-[#0c4687] border-b">
            ...
            <tbody>
              {patient.documents.map((doc, i) => (
                <tr key={i} className="border-b text-gray-700">
                  ...
                  <td><a href={doc.lien}><Download size={18} /></a></td>
                </tr>
              ))}
            </tbody>
          </table>
